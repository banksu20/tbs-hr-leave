...
const schema=z.object({action:z.enum(['preview','apply']),sourceYear:z.number().int().min(2000).max(2099),
  reconcile:z.boolean().optional(),expiresOn:z.string().refine(isPlainDate),token:z.string().optional(),
  overrides:z.array(z.object({userId:z.string().trim().min(1).max(200),carriedOver:z.number().min(0).max(365).multipleOf(0.25),expiresOn:z.string().refine(isPlainDate).nullable(),note:z.string().max(1000),annualTotal:z.number().min(0).max(365).multipleOf(0.25).optional(),sickTotal:z.number().min(0).max(365).multipleOf(0.25).nullable().optional(),personalTotal:z.number().min(0).max(365).multipleOf(0.25).optional()})).max(500).default([])})
  .refine(r=>new Set(r.overrides.map(o=>o.userId)).size===r.overrides.length,{message:'Each employee can only appear once'})
  .refine(r=>r.overrides.every(o=>o.expiresOn===null?r.reconcile===true:o.expiresOn.startsWith(String(r.sourceYear+1))),{message:'Employee expiry must be in the target year'})
